...
/*
 * 매칭 : 최종처리 단계에서 요소들이 특정 조건에 만족한느지 조사하는 역할
 * - allMatch() : 모든 요소들이 매개 값으로 주어진 Predicate의 조건을 만족하는지 조사
 * - anyMatch() : 최소한 한개의 요고가 매개 값으로 주어진 Predicate의 조건을 만족하는지 조사
...
//		a.method1();
	...
	public void method1() {
		...
		result = Arrays.stream(values).allMatch(value -> value % 2 == 0); //all 모든요소
		System.out.println(result);
		
		result = Arrays.stream(values).anyMatch(value -> value % 3 == 0); //any 하나라도 있으면
		System.out.println(result);
		
		result = Arrays.stream(values).noneMatch(value -> value % 5 ==0); // none아닌것을 체크
		System.out.println(result);
	}
