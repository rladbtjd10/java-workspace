 ...
 * 
 * Stream API
 * - 스트림은 자바 8부터 추가된 기능으로 컬렌션(배열)의 저장 요소들을 하나씩 참조해서 람다식으로 처리할 수 있도록 해주는 반복자
...
//		c.method2();
	...
	public void method1() {
		...
		IntStream stream = IntStream.range(1, 10); // 1~9까지
		int sum = stream.sum();
		System.out.println("range의 sum : " + sum); //45
		
		//첫 번째 매개 값 ~ 두 번째 매개 값 까지의 값을 요소로 가지는 스트림 객체를 생성
		stream = IntStream.rangeClosed(1, 10); // 1~10까지
		sum = stream.sum();
		System.out.println("rangeClosed 의 sum : " + sum); //55
	}
