 ...
 * --> 메모리 부족(OutOfMemoryError) 또는 스택오브플로우(StackOverflowError)
...
public class A_TryCatch {
	 ...
	 * } [finally {
	 *    // 예외가 발생하거나 발새하지 않거나 무조건 실행
	 *    // finally 주로 자원을 반납하는 코드... close() 
	 * }]
	 * 
	 * 
	 * - printStackTrace() : 예외 발생 당시의 호출스택(Call Stack)에 있었던 메서드의 정보와 예외 메서지를 화면에 출력
	 * - getMessage() : 발생한 예외클래스에 객체에 저장된 메세지를 얻을 수 있음
	 * 
	 * 
	 */
