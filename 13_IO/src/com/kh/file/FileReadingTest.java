...
//		f.method2(); // 다른 파일로 출력!
	...
	public void fileSave() {
		...
		try(FileWriter fw = new FileWriter(fileName)) { // true인 경우 새로운게 계속 저장
			
			fw.write("IO 재미있으신가요?\n");
			fw.write("아닌가요? ㅠㅠ\n");
			fw.write("그래도 계속 해주세요!\n");
			
			fw.flush(); //강제로 자료를 출력 - 마지막에 꼭 추가해줘야됨 
	...
	public void method1() {
		...
		try(BufferedReader br = new BufferedReader(new FileReader(fileName))) {
			...
			while((line = br.readLine()) != null) {
				System.out.println(line);  //line한줄 한줄 단위로 오는 것 
			}
	...
	public void method3() {
		...
		try {
			br = new BufferedReader(new FileReader(fileName));
			pw = new PrintWriter(new FileWriter(outfileName, true), true); //실행결과값 result.txt에 나옴
