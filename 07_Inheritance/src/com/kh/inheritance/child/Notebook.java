 ...
 * [표현법]
 * 
 * [접근제어자] class 자식 클래스 extends 부모 클래스{
 * 
 *            //멤버변수
 *            //생성자
 *            //메서드
 ...
 * 
 * 상속의 특징
 * - 클래스 간에 상속은 단일 상속만 가능 //단일 상속 : 부모클래스 하나에서만 상속가능
 * - 명시되어 있지 않아도 모든 클래스는 Object 클래스를 상속
 *     -> Object 클래스에서 제공하는 메서드들을 오버라이딩하면 메서드 재정의가 가능
 * - 부모 클래스의 생성자, 초기화 블록은 상속되지 않는다.
 *     ->자식 클래스 생성 시에 부모 클래스 생성자가 먼저 실행
 * - 부모의 private멤버는 상속이 되지만 직접 접근이 불가능 
...
public class Notebook extends Product { /*extends Object*/
	...
	public Notebook(String brand, String pCode, String name, int price, String cpu) {
		...
		*/
		 ...
		 *  -부모 클래스에서 setter 메서드를 제공하면 자식 객체에서 부모의 setter 메서드를 필드의 초기화가 가능
		/*
