...
//		a.method1();
	...
	public void method2() {
		...
		set.add(new Person("아오이 무네타카", 43));
		set.add(new Person("김민재", 44));
		set.add(new Person("마동석", 52));
		set.add(new Person("이준혁", 39));
		
		/*
		 * 객체의 주소갑을 가지고 비교하기 때문에 모두 다른 객체로 인식해서 중복제거되지 않음
		 * -->Person 클래스에서 hashCode(), equals() 메서드 재정의
		 * 
		 */
		
		System.out.println(set); //중복제거 안되고 추가됨(객체는 생성될때마다 주소값이 달라지니 실제로도 같더라도 다르게 인식한 것)
		System.out.println(set.size());
