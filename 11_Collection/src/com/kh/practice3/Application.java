...
public class Application {
		...
		try {
			while (check) {
				...
				case 9:
					...
					break;
				default:
					throw new Exception(); // 있는 숫자가 아닌거 입력했을때 "잘못 입력하셨습니다. 다시 입력해주세요. "를 나오게 하기위해서
				}
			}
		} catch (Exception e) {
	...
	public void adminMenu() throws Exception {
		...
		while (check) {
			...
			case 9:
				...
				break;
			default:
				throw new Exception(); // 에러뜨면 throw눌러서 위로 던져버리기
			}
		}
	}
	...
	public void addNewKind() {
		...
		try { // 번호를 잘못입력하면 과일채소견과 선택하는 메뉴로 돌아가기위해 트라이캐치 여기서 한번더 써준것
	...
	public void printFarm() {
		...
		while (it.hasNext()) {
			Farm key = it.next();
			System.out.printf("%s : %s(%d개)\n", key.getKind(), key.getName(), hMap.get(key)); // "종류 : 이름(n개)"
